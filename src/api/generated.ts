 ...
 *
 * EXCEPTION (whitelabel Phase 2, 2026-09-22): `org_logo_url` on `AuthenticatedUser`, the
 * `BrandingResponse`/`BrandingUpdateRequest`/`LogoConfirmRequest`/`LogoPresignRequest`/
 * `LogoPresignResponse` schemas, the four `/dashboard/settings/branding*` paths, and their four
 * `operations` entries were hand-added to match kayla-backend's `feature/whitelabel` branch
 * (`src/kayla/branding/schemas.py`, `src/kayla/branding/router.py`) field-for-field, because this
 * environment has no Python 3.13+ interpreter (`kayla-backend/pyproject.toml` requires it) to run
 * `uv run python scripts/export_openapi.py` and regenerate `kayla-backend/openapi.json` first.
 * `npm run codegen` reads that committed file, not a live server, so it could not pick these up on
 * its own. Re-run the real pipeline — `export_openapi.py` in kayla-backend, then
 * `npm run codegen` here — the next time both repos' branches merge, and delete this paragraph
 * once that diff shows no drift.
 */
...
export interface paths {
    ...
    "/dashboard/settings/branding": {
        ...
        /**
         * Read this organisation's whitelabel branding
         * @description The effective display name (this org's own override, or its default name if never customised) and logo. Never a 404 for an org that has not set either.
         */
        get: operations["get_branding_dashboard_settings_branding_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /**
         * Update this organisation's display name, or remove its logo
         * @description Partial update — every field is optional and an omitted one is left unchanged. `remove_logo=true` reverts to no logo. Uploading a *new* logo goes through `POST .../logo/presign` and `.../logo/confirm` instead of this endpoint.
         */
        patch: operations["update_branding_dashboard_settings_branding_patch"];
        trace?: never;
    };
    "/dashboard/settings/branding/logo/confirm": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        put?: never;
        /**
         * Confirm a completed logo upload
         * @description Called once the browser's presigned PUT has completed. Re-verifies the uploaded object's real size against the server-side limit before recording it — a client's declared size at presign time is never trusted on its own.
         */
        post: operations["confirm_logo_upload_dashboard_settings_branding_logo_confirm_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
    ...
    "/dashboard/settings/branding/logo/presign": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        put?: never;
        /**
         * Request a presigned upload URL for this organisation's logo
         * @description Validates content type and declared size before minting a presigned PUT URL — this API process never receives the file's bytes. Call POST .../logo/confirm once the PUT completes; the upload is not reflected in GET .../branding until then.
         */
        post: operations["presign_logo_upload_dashboard_settings_branding_logo_presign_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
...
export interface components {
    schemas: {
        ...
        AuthenticatedUser: {
            ...
            /**
             * Org Logo Url
             * @description This organisation's whitelabel logo, if it has uploaded one — for the dashboard sidebar mark and the browser favicon. A public, unauthenticated URL (never a signed one), so it is safe to render directly in an <img> with no further request. Populated by GET /auth/me and by POST /orgs/signup/complete; null on login and refresh (which do not perform this read) and for an org that has never uploaded a logo.
             */
        ...
        /**
         * BrandingResponse
         * @description `GET`/`PATCH`/`.../logo/confirm`'s shared response.
         *
         *     `display_name` is always the **effective** name — this org's own override if it has set one,
         *     otherwise `Org.name` — never null, so a caller renders it directly with no fallback logic of
         *     its own. `has_custom_branding=False` means this org has never set a display name or logo.
         */
        BrandingResponse: {
            /**
             * Display Name
             * @description Effective display name shown to this org's employees.
             */
            display_name: string;
            /** @description False when this org has never set a display name override or a logo. */
            has_custom_branding: boolean;
            /**
             * Logo Url
             * @description Null until a logo has been uploaded.
             */
             ...
             * Updated At
             * Format: date-time
             * @description When branding was last written. Null until the first write.
             */
            updated_at?: string | null;
        };
        /**
         * BrandingUpdateRequest
         * @description `PATCH /dashboard/settings/branding` — a partial update. An omitted field is left
         *     unchanged. `remove_logo=true` clears the logo (reverting to no logo) and
         *     `reset_display_name=true` clears a custom display name (reverting to the organisation's own
         *     name) — either can be combined with a `display_name` edit or used alone. Uploading a *new*
         *     logo goes through `POST .../logo/presign` + `.../logo/confirm` instead, never through this
         *     endpoint.
         */
        BrandingUpdateRequest: {
            /**
             * Display Name
             * @description New display name. Omit to leave unchanged. Mutually exclusive with reset_display_name.
             */
            display_name?: string | null;
            /**
             * Remove Logo
             * @description Set true to remove the current logo, if any.
             * @default false
             */
            remove_logo?: boolean;
            /**
             * Reset Display Name
             * @description Set true to clear a custom display name, reverting to the organisation's own name.
             * @default false
             */
            reset_display_name?: boolean;
        };
        /**
         * ChatCitation
        ...
        /**
         * LogoConfirmRequest
         * @description `POST /dashboard/settings/branding/logo/confirm` — called once the browser's presigned PUT
         *     has completed.
         */
        LogoConfirmRequest: {
            /**
             * Storage Key
             * @description The storage_key returned by .../presign.
             */
            storage_key: string;
        };
        /**
         * LogoPresignRequest
         * @description `POST /dashboard/settings/branding/logo/presign`.
         */
        LogoPresignRequest: {
            /**
             * Content Type
             * @description Must be image/png, image/jpeg or image/webp.
             * @example image/png
             */
            content_type: string;
            /**
        ...
        /**
         * LogoPresignResponse
         * @description A presigned PUT URL the browser uploads directly to — this API process never receives the
         *     file's bytes.
         */
        LogoPresignResponse: {
            /**
            ...
            /**
             * Max Size Bytes
             * @description Uploads larger than this are rejected at confirm.
             */
            max_size_bytes: number;
            /**
             * Storage Key
             * @description Pass this to POST .../logo/confirm after the PUT.
             */
            storage_key: string;
            /**
             * Upload Url
...
export interface operations {
    ...
    get_branding_dashboard_settings_branding_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        responses: {
            ...
            /** @description Signed in, but not as an org owner. */
            403: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    update_branding_dashboard_settings_branding_patch: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        responses: {
            ...
            /** @description The field is out of bounds, the patch is empty, or the logo failed content-type/size validation. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    confirm_logo_upload_dashboard_settings_branding_logo_confirm_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        requestBody: {
            content: {
                "application/json": components["schemas"]["LogoConfirmRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
    ...
    presign_logo_upload_dashboard_settings_branding_logo_presign_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        requestBody: {
            content: {
                "application/json": components["schemas"]["LogoPresignRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                ...
                content: {
                    "application/json": components["schemas"]["LogoPresignResponse"];
                };
            };
            /** @description The credential is missing, expired, revoked or not ours. */
            401: {
